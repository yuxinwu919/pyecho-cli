%Wss assembling
% ECHO options: Offset>0, SymmetryCondition=elec  
clear all;close all;
Nm=3;
dir_in='../../ECHO2D/elec/';
dir_out='../../ECHO2D/elec/';
...
for i=1:Nm,
    ...
    w=load(filename);
    w(1:2,:)=[];
    W(i+1,2:ns+1)=w(:,2)/sinh(dy*f)^2;
    WD(i,:)=w(:,2);
end;

filename=[dir_out 'Wss_odd.txt'];
save(filename,'W','-ascii');

subplot(2,1,1)
mesh(W(1,2:end)*1e3,W(2:end,1)*1e-3,WD*1e-3);
title(['Modal wakes W(k_x,s)[V/pC*m] at offset=' num2str(dy*1e3) ' mm with electric BC']);
xlabel('s[mm]');ylabel('k_x[1/mm]');
subplot(2,1,2)
mesh(W(1,2:end)*1e3,W(2:end,1)*1e-3,W(2:end,2:end)*1e-3);
title('1D modal functions W_s_s(k_x,s)[V/pC*m]');
xlabel('s[mm]');ylabel('k_x[1/mm]');
