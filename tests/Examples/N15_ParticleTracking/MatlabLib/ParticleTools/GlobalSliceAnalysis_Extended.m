function [s,I,ex,ey,me,se,gamma0,emitxn,emityn]=GlobalSliceAnalysis_Extended(PD,q1,Mslice,Mcur,p,iter)
%[s,I,ex,ey,me,se,gamma0,emitxn,emityn]=GlobalSliceAnalysis_Extended(PD,q1,Mslice,Mcur,p,iter)
PhysConsts;
PD=sortrows(PD,3); z=PD(:,3);
[mx mxs mxx mxxs mxsxs emittx]=SliceAnalysis (z,PD(:,1),PD(:,4),Mslice,true);
[my mys myy myys mysys emitty]=SliceAnalysis (z,PD(:,2),PD(:,5),Mslice,true);
[mE mEs mEE mEEs mEsEs emittE]=SliceAnalysis (z,PD(:,3),PD(:,6),Mslice,true);
mE=mEs;
sE=sqrt(mEsEs); 
...
[z ind]=unique(z);emittx=emittx(ind);emitty=emitty(ind);sE=sE(ind);mE=mE(ind);
